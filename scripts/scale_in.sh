...
LINE=$(redis-cli cluster nodes | grep -F -w "$MASTER_ID" | head -1)
if [[ -z "$LINE" ]]; then
...
sleep 2
if redis-cli cluster nodes | grep -F -w "$MASTER_ID" | grep -qE 'connected [0-9]'; then
...
for rid in $REPLICA_IDS; do
  RADDR=$(redis-cli cluster nodes | grep -F -w "$rid" | awk '{print $2}' | cut -d@ -f1)
